  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const initialMessages: Message[] = [
        { id: 'init-1', text: `Merhaba ${userName}, ben İtemSatış Asistan. 👋`, sender: 'bot', timestamp: getCurrentTime() },
        { id: 'init-2', text: 'İtemSatış hakkında nasıl yardımcı olabilirim? 😊', sender: 'bot', timestamp: getCurrentTime() }
      ];
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <motion.div 
          ...
          className="bg-[#232736] w-full max-w-2xl h-[600px] rounded-2xl border border-white/10 shadow-2xl flex flex-col overflow-hidden"
        >
          {/* Header */}
          <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#1a1d27]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-bold text-sm">İtemSatış Asistan</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-hide" ref={scrollRef}>
              ...
              <div className="space-y-2">
                {faqQuestions.map(q => (
                  <button 
                    ...
                    className="w-full text-left p-3 rounded-xl bg-[#1a1d27] border border-white/5 hover:border-[#5b68f6]/50 hover:bg-[#5b68f6]/5 transition-all text-gray-300 text-xs flex items-center gap-3 group"
                  >
            ...
            <div className="relative flex items-center justify-center py-4">
              ...
              <span className="relative bg-[#232736] px-4 text-[10px] text-gray-500 uppercase tracking-widest">Konuşma Başladı</span>
            </div>
            ...
            {messages.map((msg) => (
              ...
              >
                <div className={`max-w-[80%] flex gap-3 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border ${msg.sender === 'user' ? 'bg-[#5b68f6] border-white/10' : 'bg-[#1a1d27] border-white/10'}`}>
                    {msg.sender === 'user' ? <User className="w-4 h-4 text-white" /> : <Bot className="w-4 h-4 text-[#5b68f6]" />}
                  </div>
                  <div className="space-y-1">
                    <div className={`p-3 rounded-2xl text-sm leading-relaxed ${msg.sender === 'user' ? 'bg-[#5b68f6] text-white rounded-tr-none' : 'bg-[#1a1d27] text-gray-300 rounded-tl-none border border-white/5'}`}>
                      {msg.text}
                    </div>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="flex gap-3">
                  <div className="w-8 h-8 rounded-full bg-[#1a1d27] border border-white/10 flex items-center justify-center">
                    <Bot className="w-4 h-4 text-[#5b68f6]" />
                  </div>
                  <div className="bg-[#1a1d27] border border-white/5 p-3 rounded-2xl rounded-tl-none flex gap-1">
          ...
          {/* Input Area */}
          <div className="p-4 bg-[#1a1d27] border-t border-white/5">
            <form 
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              <input 
                ...
                className="w-full bg-[#232736] border border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm text-white focus:outline-none focus:border-[#5b68f6] transition-colors"
              />
              <button 
                type="submit"
